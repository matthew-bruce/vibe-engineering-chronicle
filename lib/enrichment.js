 ...
 * Only human-confirmed evergreen cards are permanently excluded.
 */
 ...
 * Respects _source locks — never overwrites fields confirmed by humans.
 * In scheduled mode, writes an audit prefix to card_ai_summary when
 * relevance drifts.
 *
 * @param {object} card     - existing card record with _source fields
 * @param {object} response - validated enrichment result
 * @param {object} opts     - { isScheduled?: boolean }
 * @returns {object}        - update object safe to pass to supabase .update()
 */
export function buildEnrichmentUpdate(card, response, opts = {}) {
  const { isScheduled = false } = opts;
  ...
  // Audit note for scheduled relevance drift
  let summary = response.card_ai_summary;
  if (
    isScheduled &&
    card.card_relevance_source !== 'human' &&
    card.card_relevance &&
    card.card_relevance !== response.card_relevance
  ) {
    summary = `Relevance updated ${card.card_relevance} → ${response.card_relevance}. ${summary}`;
  }
  update.card_ai_summary = summary;

  // card_impact — only update if not human-confirmed
  if (card.card_impact_source !== 'human') {
    update.card_impact        = response.card_impact;
    update.card_impact_source = 'ai';
  }

  // card_relevance — only update if not human-confirmed
  if (card.card_relevance_source !== 'human') {
    update.card_relevance        = response.card_relevance;
