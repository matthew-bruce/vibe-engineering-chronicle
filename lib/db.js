...
import { buildCardSnapshot, buildRestorePayload, getVersionsToPrune } from './versioning.js';
...
export async function initLookups() {
  const [catsRes, themesRes, projectsRes] = await Promise.all([
    supabase.from('categories').select('category_id, category_name, category_colour, category_glyph, category_description'),
  ...
  for (const c of catsRes.data) {
    ...
    if (slug) {
      ...
      cats[slug] = { label: c.category_name, color: c.category_colour, glyph: c.category_glyph, description: c.category_description ?? '' };
    }
  }
...
function mapTimelineCard(row) {
  ...
  return {
    id:        row.card_id,
    date:      row.card_event_date ?? row.card_created_at.slice(0, 10),
    ...
    format:          row.card_format          ?? null,
    impactSource:    row.card_impact_source   ?? null,
    relevance:       row.card_relevance       ?? null,
    relevanceSource: row.card_relevance_source ?? null,
    aiThemes:        Array.isArray(row.card_ai_themes) ? row.card_ai_themes : [],
    aiAudience:      row.card_ai_audience     ?? null,
...
const CARD_SELECT = `
  ...
  card_impact, card_impact_source, card_audience,
  card_relevance, card_relevance_source,
  card_ai_themes, card_ai_audience,
  card_ai_summary, card_enriched_at,
  card_event_date, card_source, card_format, card_created_at,
...
export async function updateTimelineEntry(id, fields) {
  await snapshotCard(id, 'user', 'Manual edit');
  ...
  const { error: cardErr } = await supabase
    ...
    .update({
      ...
      card_format:     fields.format   || null,
...
/**
 * Confirm a single AI-suggested field as human-approved.
 * field: 'impact' | 'relevance'
 * Sets the corresponding _source column to 'human', locking it from future AI overwrites.
 */
export async function confirmCardField(id, field) {
  const col = field === 'impact' ? 'card_impact_source' : 'card_relevance_source';
  const { error } = await supabase
    .from('cards')
    .update({ [col]: 'human' })
    .eq('card_id', id);
...
/**
 * Snapshot the current state of a card into card_versions before any mutation.
 * If the card already has MAX_VERSIONS snapshots, the oldest is pruned.
 */
export async function snapshotCard(cardId, versionedBy, versionNote) {
  // 1. Fetch current card state + related rows in parallel
  ...
  const snapshot = buildCardSnapshot(cardRes.data, catsRes.data, themesRes.data, sectionsRes.data);
  ...
  // 4. Prune oldest if we are over the limit
  const { data: allVersions, error: listErr } = await supabase
    .from('card_versions')
  ...
  const toPrune = getVersionsToPrune(allVersions);
  if (toPrune.length > 0) {
    const { error: pruneErr } = await supabase
      .from('card_versions')
      .delete()
      .in('version_id', toPrune);
    if (pruneErr) throw pruneErr;
  }
}

/**
 * Load the version list for a card (most recent first), without snapshot data.
 * Used to render the version history panel.
 */
export async function loadCardVersions(cardId) {
  const { data, error } = await supabase
    .from('card_versions')
    .select('version_id, version_number, versioned_at, versioned_by, version_note')
    .eq('card_id', cardId)
    .order('version_number', { ascending: false });
  if (error) throw error;
  return data.map(v => ({
    ...
    versionedAt: v.versioned_at,
    versionedBy: v.versioned_by,
    note:        v.version_note ?? '',
  }));
}
 ...
 * Always snapshots the current state first so the restore itself is undoable.
 */
export async function restoreCardVersion(cardId, versionId) {
  // 1. Snapshot current state so user can undo the restore
  await snapshotCard(cardId, 'user', 'Pre-restore snapshot');

  // 2. Load the target version snapshot
  ...
  const { cardFields, categories, themes, sections } = buildRestorePayload(versionRow.card_snapshot);

  // 3. Update main card fields
  const { error: cardErr } = await supabase.from('cards').update(cardFields).eq('card_id', cardId);
  if (cardErr) throw cardErr;

  // 4. Replace categories
  await supabase.from('card_categories').delete().eq('card_id', cardId);
  if (categories.length > 0) {
    const { error: catErr } = await supabase
      .from('card_categories')
      .insert(categories.map(catId => ({ card_id: cardId, category_id: catId })));
    if (catErr) throw catErr;
  }

  // 5. Replace themes
  await supabase.from('card_themes').delete().eq('card_id', cardId);
  if (themes.length > 0) {
    const { error: themeErr } = await supabase
      .from('card_themes')
      .insert(themes.map(themeId => ({ card_id: cardId, theme_id: themeId })));
    if (themeErr) throw themeErr;
  }

  // 6. Soft-delete existing sections then re-insert from snapshot
  await supabase
    .from('card_sections')
    .update({ section_deleted_at: new Date().toISOString() })
    .eq('card_id', cardId)
